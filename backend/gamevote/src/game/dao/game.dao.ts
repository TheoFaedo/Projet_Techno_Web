...
export class GameDao{
    ...
    getGames(sort?: {name: string, order: SortOrder} | null, filters?: RequestFilter[] | null) {
        ...
        if(filters){
            filters.forEach(filter => {
                if(filter.operator == "="){
                    if(filter.variable == "genres" || filter.variable == "platforms" || filter.variable == "game_modes"){
                        query.where(filter.variable).all(filter.value.split(",").map(id => Number(id)));
                    }else{
                        query.where(filter.variable).equals(filter.value);
                    }
                }else if(filter.operator == "~"){
                    query.where(filter.variable).regex(new RegExp(filter.value.slice(2, -2), "i"));
                }
            })
        }
    ...
    async create(igbdId: number, steamAppId: number, price: number, name: string, first_release_date: number, game_modes: number[], platforms: number[], genres: number[]) {
