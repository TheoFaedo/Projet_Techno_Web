...
export class IgdbService {
    ...
    async authenticate(){
        ...
        let response = await fetch(`https://id.twitch.tv/oauth2/token?client_id=${process.env.IGDB_APP_ID}&client_secret=${process.env.IGDB_APP_SECRET}&grant_type=client_credentials`, {
            method: 'POST',
        })
    ...
    isAuthenticated(){
        ...
        if((new Date().getTime() - this.last_time_auth.getTime())/1000 > this.token_duration){
            return false;
        }
        return true;
    }
